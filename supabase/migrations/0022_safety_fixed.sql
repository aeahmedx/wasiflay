...
-- 0. Columns first
--
--    is_active_user() below references profiles.deleted_at, and Postgres
--    validates function bodies at creation time — so the column has to
--    exist before the function that reads it. Getting this order wrong
--    fails the whole transaction, which is what happened the first time.
-- ---------------------------------------------------------------------
...
--    people's conversations along with it. The column itself is added
--    at the top, before is_active_user() needs it.
-- ---------------------------------------------------------------------

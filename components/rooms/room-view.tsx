...
export function RoomView({
  ...
  const { messages, authors, connection } = useRoomMessages(
    room.id,
    initialMessages
  );
  ...
  // Drop optimistic entries once the real row arrives.
  useEffect(() => {
    if (pending.length === 0) return;
    const bodies = new Set(
      messages.filter((m) => m.author_id === userId).map((m) => m.body ?? "")
    );
    setPending((current) =>
      current.filter((p) => p.failed || !bodies.has(p.body))
    );
  }, [messages, userId, pending.length]);
  ...
  return (
    <div className="flex flex-col h-dvh bg-stone-50">
      <header className="flex items-center justify-between border-b border-stone-200 bg-white px-4 py-3">
        <div>
          <h1 className="font-semibold text-stone-900">{room.name}</h1>
          {presence !== null && (
        ...
        {messages.map((m) => {
          ...
          return (
            <div key={m.id} className="flex flex-col">
              ...
              <p
                className="text-stone-900 whitespace-pre-wrap break-words"
                dir="auto"
              >
                {m.body}
              </p>
